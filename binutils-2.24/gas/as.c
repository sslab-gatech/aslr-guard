...
static bool flag_alsr_only_preprocess = false;
static char *flag_aslr_temp_output = NULL;
...
parse_args (int * pargc, char *** pargv)
{
  ...
  enum option_values
    {
      ...
      OPTION_ASLR_ONLY_ASM_PREPROCESS,
      OPTION_ASLR_SAVE_TEMPS
  ...
  static const struct option std_longopts[] =
  {
    ...
    ,{"aslr-only-asm-preprocess", no_argument, NULL, OPTION_ASLR_ONLY_ASM_PREPROCESS}
    ,{"aslr-save-temps", required_argument, NULL, OPTION_ASLR_SAVE_TEMPS}
  };
...
This program has absolutely no warranty.\n"));
  ...
  case OPTION_ASLR_ONLY_ASM_PREPROCESS:
    flag_alsr_only_preprocess = true;
    break;

  case OPTION_ASLR_SAVE_TEMPS:
    flag_aslr_temp_output = xstrdup(optarg);
    break;
...
/* testing is_insn() */
static
void _test_insn(void)
{
  case_t cases[] = {
    {false, "        .file   \"test.c\""       },
    {false, "        .section        .rodata"  },
    {false, ".LC0:"                            },
    {false, "        .string \"Hello World\""  },
    {false, "        .text"                    },
    {false, "        .globl  hello"            },
    {false, "        .type   hello, @function" },
    {false, "hello:"                           },
    {false, ".LFB0:"                           },
    {false, "        .cfi_startproc"           },
    {true , "        pushq   %rbp"             },
    {false, "        .cfi_def_cfa_offset 16"   }
  };

  /* NOTE. need better assert routine */
  size_t i;
  for (i = 0; i < sizeof(cases) / sizeof(cases[0]); i++)
    aslr_assert_equal(is_insn(cases[i].line), cases[i].expected);
}

/* testing potential_func_addr_taken() */
static
void _test_func_addr(void)
{
  char *func_syms[] = {"foo", "bar"};
  struct s_asm_data meta = {
    .func_syms     = func_syms,
    .func_syms_num = sizeof(func_syms) / sizeof(func_syms[0]),
    .obj_syms      = NULL,
    .obj_syms_num  = 0,
  };
  ...
  size_t i;
  for (i = 0; i < sizeof(cases) / sizeof(cases[0]); i++)
    aslr_assert_equal(potential_func_addr_taken(cases[i].line, &meta),
                      cases[i].expected);
}

static
void _test_parsing(void) {
  char *insn = "movq    $0x400690,%rcx";

  op_t op;
  get_opcode(insn, op);
  aslr_assert_equal_str(op, "movq");
  aslr_assert(get_optype(insn, 3) == 'q');

  get_oparand(insn, 1, op);
  aslr_assert(strstr(op, "$0x400690"));
  get_oparand(insn, 2, op);
  aslr_assert(strstr(op, "%rcx"));

  insn = "call *(%r12,%rbx,8)";
  get_oparand(insn, 1, op);
  aslr_assert_equal_str(op, "*(%r12,%rbx,8)");

  char *buf = strdup("abcdefabcd");
  _str_replace(&buf, "abc", "xx");
  aslr_assert(strstr(buf, "xx"));
  _str_replace(&buf, "xx", "yyyy");
  aslr_assert(strstr(buf, "yyyy"));
}
...
#ifdef AG_ENABLE
  {
    /* wondering if no one pass more than one asm file in the past */
    //gas_assert(argc == 2);

    /* XXX. check no direct invocation, since it directly modifies
       asm source code */

    /* skip 'as' and iterate over asm files */
    int i;
    for (i = 1; i < argc; i ++) {
      struct s_asm_data asm_data = {
        .filename = strdup(argv[i]),
        .temp_filename = flag_aslr_temp_output,
      };
      if (!load_asm(&asm_data)) {
        err(1, "failed to load asm lines");
      }

      load_syms(&asm_data);
      process_asm(&asm_data);
      flush_asm(&asm_data);
      free_asm_data(&asm_data);
    }

    if (flag_alsr_only_preprocess)
      exit(EXIT_SUCCESS);
  }
#endif

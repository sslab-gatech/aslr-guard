...
expand_virtual_init (tree binfo, tree decl)
{
  ...
  /*
  // ag-note: mark vtbl pointer definition for pointer encryption in linker
  tree void_ftype = build_function_type_list (void_type_node, NULL_TREE);
  tree fn = build_lang_decl (FUNCTION_DECL, get_identifier ("_AG_VPTR_DEF_"), void_ftype);
  finish_expr_stmt(build_call_a(fn, 0, NULL));
  */

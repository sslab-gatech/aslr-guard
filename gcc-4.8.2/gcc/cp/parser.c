	  ...
	  /* postfix-expression ( expression-list [opt] ) */
	  {
		...
		if (BASELINK_P (fn))
		  ...
		  postfix_expression
			...
			complain));
      /*
      // ag-note: mark vtbl pointer use
      if (DECL_VIRTUAL_P(fn) || BINFO_VIRTUAL_P(fn) || IDENTIFIER_VIRTUAL_P(fn)) {
        tree void_ftype = build_function_type_list (void_type_node, NULL_TREE);
        tree vfn = build_lang_decl (FUNCTION_DECL, get_identifier ("_AG_VPTR_USE_"), void_ftype);
        finish_expr_stmt(build_call_a(vfn, 0, NULL));
		  }
      */
      }
		else

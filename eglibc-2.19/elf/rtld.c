...
static ElfW(Addr) elf_l_addr = 0;
static ElfW(Addr) __attribute_used__ internal_function
_dl_start (void *arg)
{
#ifdef AG_SAFE_STACK
  // ag-note: create regular stack
#define AG_STACK_SIZE (0x800000) //default stack size 8M
  size_t ptr_size = sizeof(void *);

  ElfW(Addr) AG_FP = (ElfW(Addr))__mmap ((caddr_t) 0, 
      //2 MB
      //(256 + 256) * GLRO(dl_pagesize),
      AG_STACK_SIZE,
      PROT_READ|PROT_WRITE, 
      MAP_ANONYMOUS|MAP_PRIVATE,
      -1, 0);
  // reserve 1 page for arg and current stack frame
  //AG_FP = AG_FP + (256 + 256 - 1) * GLRO(dl_pagesize);
  AG_FP = AG_FP + AG_STACK_SIZE - GLRO(dl_pagesize) + ptr_size;
  // ag-note: copy arg and current stack frame data 
  // to regular stack
  ElfW(Addr) AG_SP = 0,  AG_FP_old = 0;
  //size_t frame_size = ptr_size + 0x60;
  //get %rsp and %r15
  __asm__ __volatile__ (
      "\tmov\t%%rsp, %0 #_volatile_\n"
      "\tmov\t%%r15, %1 #_volatile_\n"
      :"=r" (AG_SP), "=r" (AG_FP_old)
      );
#ifdef PRINT_DEBUG_INFO
  _dl_debug_printf("AG_FP = 0x%lx, AG_SP = 0x%lx, AG_FP_old = 0x%lx\n", AG_FP, AG_SP, AG_FP_old);
#endif
  // copy current stack frame
  size_t frame_size = AG_SP - AG_FP_old - 0x10000;
#if 0
  int idx = frame_size / sizeof(ElfW(Addr));
  while(idx) {
    *((ElfW(Addr) *)AG_FP + idx - 1) = *((ElfW(Addr) *)AG_FP_old + idx - 1);
    --idx;
  }
#endif
  memcpy(AG_FP, AG_FP_old, frame_size);

  // copy arg
  // two sets of variables
  size_t arg_len = 0;
  while (*(ElfW(Addr) *)(arg + arg_len))
    arg_len += ptr_size;
  arg_len += ptr_size;
  while (*(ElfW(Addr) *)(arg + arg_len))
    arg_len += ptr_size;
...
#if 0
  AG_SP += sizeof(ElfW(Addr)); //skip return address
  //AG_FP += frame_size;//(sizeof(ElfW(Addr)) + 0x60); //a push and 0x60 stack frame
  while(arg_len) {
    *((ElfW(Addr) *)AG_FP + (frame_size / sizeof(ElfW(Addr))) + arg_len - 1) = 
      *((ElfW(Addr) *)AG_SP + arg_len - 1);
    --arg_len;
  }
#endif
  memcpy(AG_FP + frame_size, arg, arg_len);

  // reset %r15 to the random regular stack. done.
  __asm__ __volatile__ (
      "\tmov\t%0, %%r15 #_volatile_\n"
      :
      :"r" (AG_FP)
      );
...
#ifdef DO_STATISTICS
stat_ptr = (ElfW(Addr))__mmap ((caddr_t) 0, 
      GLRO(dl_pagesize),
      PROT_READ|PROT_WRITE, 
      MAP_ANONYMOUS|MAP_PRIVATE,
      -1, 0);
...
//ag-note: record start clock cycle
#ifdef TEST_LOAD_TIME
  hp_timing_t start_load_time;
  HP_TIMING_NOW (start_load_time);
#endif
...
# endif
  ...
  /*********************************/
  //ag-note: initialize safe memory
#if defined(AG_ENCODE_CP) || defined(REMAP_CODE_TO_RANDOM)
  safeMemReg = (ElfW(Addr))__mmap ((caddr_t) 0, 
      // 1st MB for remap info; remaining space for function pointer mapping
      // 2 MB in total should be enough
      (256 * 256) * GLRO(dl_pagesize), 
      PROT_READ|PROT_WRITE, 
      MAP_ANON|MAP_PRIVATE,
      -1, 0);
  assert (safeMemReg != NULL);

#ifdef DO_STATISTICS
  mkdir("/tmp/ag_out", S_IRWXU);
  pid_t pid = __getpid ();
  char pidbuf[12] = {0};
	char *pid_str = _itoa (pid, (char *)pidbuf, 10, 0);
  char stat_fname[32] = {0};
  strcpy((void *)stat_fname, "/tmp/ag_out/");
  strcpy((void *)&stat_fname[12], (void *)pid_str);
  int stat_f = open(stat_fname, O_RDWR|O_CREAT, S_IRUSR|S_IWUSR);
  if (stat_f > 0) {
    // write data into file
    _dl_dprintf(stat_f, 
        "PID:%lu\nSecureVault:0x%lx\nMapTable:0x%lx\nStatistics:0x%lx",
        pid, safeMemReg, safeMemReg + 256 * GLRO(dl_pagesize), stat_ptr);
    close(stat_f);
  }
  _dl_debug_printf("PID = %lu \n", pid);
  _dl_debug_printf("Secure vault address = 0x%lx, Random map table = 0x%lx,\ 
      Statistic data = 0x%lx\n\n", 
      safeMemReg, safeMemReg + 256 * GLRO(dl_pagesize), 
      stat_ptr);
#endif

  //set GS segment register (gs.base)
  __asm__ __volatile__ (
      "push %%rax #_volatile_\n\t"
      "push %%rdi #_volatile_\n\t"
      "push %%rsi #_volatile_\n\t"
      ...
      "push %%r8 #_volatile_\n\t"
      "push %%r9 #_volatile_\n\t"
      "mov %0, %%rsi #_volatile_\n\t"
      "mov $0x1001, %%rdi #_volatile_\n\t"
      "mov $0x9e, %%eax #_volatile_\n\t"
      "syscall #_volatile_\n\t"
      "pop %%r9 #_volatile_\n\t"
      "pop %%r8 #_volatile_\n\t"
      "pop %%r10 #_volatile_\n\t"
  ...
  *(int *)safeMemReg = 0;
  current_remapInfo_off = 0;
  // reserve 8 words in rand map
  *(size_t *)(safeMemReg + REMAP_INFO_SIZE) = sizeof(size_t) * 8;
  // save file handler of /dev/urandom at %gs:0x100008
  *(size_t *)(safeMemReg + REMAP_INFO_SIZE + sizeof(size_t)) 
    = open("/dev/urandom", 0); // O_RDONLY: 0
  // save location for saving random value at %gs:0x100010
  // random value will be save at %gs:0x10001c
  *(size_t *)(safeMemReg + REMAP_INFO_SIZE + 2 * sizeof(size_t)) 
    = safeMemReg + REMAP_INFO_SIZE + 3 * sizeof(size_t) + 4;

  elf_path = (char *)*(ElfW(Addr) *)(arg + sizeof(char *));
  // remap dynamic loader
  _dl_remap_loader(elf_path, bootstrap_map.l_addr);

  // save offset returned by remap function
  __asm__ __volatile__(
      "push %rax #_volatile_"
      );
#endif
  ...
  if (bootstrap_map.l_addr || ! bootstrap_map.l_info[VALIDX(DT_GNU_PRELINKED)])
    {
	 ...
	 data access using the global offset table.  */
      // ag-note: OS has done the lazy relocaton for dynamic loader
      // need to update these relocation entries: 0->1
      ELF_DYNAMIC_RELOCATE (&bootstrap_map, 1, 0, 0);
    }
  bootstrap_map.l_relocated = 1;


  /*********************************/
  // ag-note: jump to remaped loader code
#ifdef REMAP_CODE_TO_RANDOM
  __asm__ __volatile__(
      "pop %rax #_volatile_\n\t"
      "push %rbx #_volatile_\n\t"
      "lea 0x6(%rip), %rbx #_volatile_\n\t"
      "add %rbx, %rax #_volatile_\n\t"
      "pop %rbx #_volatile_\n\t"
      "jmp *%rax #_volatile_"
      );

  // make original loader non-executable
  ElfW(Addr) oldCodeBase = 
    *(ElfW(Addr)*)(safeMemReg + sizeof(int) + sizeof(ElfW(Addr)));
  size_t codeSize = 
    *(ElfW(Addr)*)(safeMemReg + sizeof(int) + 5 * sizeof(ElfW(Addr)));
  off_t codepad = oldCodeBase % GLRO(dl_pagesize); 
#ifndef ENABLE_DEBUG_MODE
  __mprotect((void *)(oldCodeBase - codepad),  codeSize + codepad, PROT_READ);
#endif
#endif
  /*********************************/
...
# define ELF_MACHINE_START_ADDRESS(map, start) (start)
    ...
    /*********************************/
    // ag-note: get new entry address
#ifdef AG_ENCODE_CP
    entry = adjust_encode_code_addr (entry, MAY_ENCODE, 0);
#endif

#ifdef REMAP_CODE_TO_RANDOM
    // moved offset of loader
    off_t movedLd = get_offset_remap_code_l_addr (bootstrap_map.l_addr);

    // adjust return value (address of _dl_start_user) before return
    __asm__ __volatile__(
        //"push %%rbx #_volatile_\n\t"
        //"lea 0x0(%%rsp), %%rbx #_volatile_\n\t" //0x80
        "add %0, (%%rsp) #_volatile_\n\t"
        //"pop %%rbx #_volatile_\n\t"
        :
        : "r" (movedLd)
    );
  ...
  //print loading time for evaluation
#ifdef TEST_LOAD_TIME
    print_time_now();

    hp_timing_t end_load_time, total_load_time;
    HP_TIMING_NOW (end_load_time);

    HP_TIMING_DIFF (total_load_time, start_load_time, end_load_time);
    print_statistics (&total_load_time);
#endif
#ifdef DO_STATISTICS
    _dl_debug_printf("@@@ Number of all static code pointers: %lu\n", *stat_ptr);
    _dl_debug_printf("@@@ Number of GOTPLT code pointers: %lu\n", *(stat_ptr+1));
    _dl_debug_printf("@@@ Number of fixuped code pointers: %lu\n", *(stat_ptr+2));
    _dl_debug_printf("@@@ Number of data-access patches: %lu\n", *(stat_ptr+3));
    _dl_debug_printf("@@@ Number of all relocations: %lu\n", *(stat_ptr+4));
#endif
  /*********************************/
...
lookup_doit (void *a)
{
  ...
  if (ref != NULL)
    args->result = adjust_encode_code_addr(DL_SYMBOL_ADDRESS (l, ref), MAY_ENCODE, 0);
}
...
#define VERNEEDTAG (DT_NUM + DT_THISPROCNUM + DT_VERSIONTAGIDX (DT_VERNEED))
  ...
  // ag-note: remap executable itself
  // ASLR need to be enabled
  _dl_remap_module(NULL, elf_path, main_map->l_addr);
  elf_l_addr = main_map->l_addr;
  ...
  if (prelinked)
    ...
    // ASLR need to be enabled
    if(i == 0) {
      _dl_remap_module(NULL, elf_path, l->l_addr);
      elf_l_addr = l->l_addr;
    }
    */

...
call_init (struct link_map *l, int argc, char **argv, char **env)
{
  ...
  */
  if (l->l_info[DT_INIT] != NULL) {
    // ag-note: patch address
    off_t moved = 0;
#ifdef REMAP_CODE_TO_RANDOM
    moved = get_offset_remap_code_l_addr(l->l_addr);
#endif

    DL_CALL_DT_INIT(l, encrypt_function_pointer(l->l_addr + l->l_info[DT_INIT]->d_un.d_ptr + moved), argc, argv, env);
  }

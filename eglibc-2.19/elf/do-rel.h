...
# endif
  ...
  /*********************************/
  //Kangjie get current remapInfo offset to speed up lazy relocation
#ifdef USE_ASLR_GUARD
  int idx = 0;
  while (idx < *(int *)safeMemReg) { 
    if (*(ElfW(Addr)*)(safeMemReg + sizeof(int) 
          + idx * sizeof(struct remapInfo)) == l_addr) {
      current_remapInfo_off = sizeof(int) + idx * sizeof(struct remapInfo);
      break;
    }
    ++idx;
  }

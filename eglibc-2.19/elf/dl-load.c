...
/*********************************/
// ag-note: utils functions
// rewrite this util functions to avaid fPIC compilation
const char *StrStr(const char *string, const char *substring)
{
  char *a, *b = (char *)substring;
...
// ag-note: remap code and gotplt table
static bool _dl_remap_code_gotplt(
  ...
  /*************************************************************************/
  // remap code sections as well as .got and .got.plt to a randomized address
  // section between code section and got section will be mapped too,
  // need to rely on compiler to remove them
  const ElfW(Ehdr) ehdr;
  const ElfW(Shdr) shdr;
  int idx = 0;
  char* SectNames = NULL;
  off_t ori_off = lseek(fd, 0, SEEK_CUR); //get current offset
  off_t fsize = lseek(fd, 0, SEEK_END);
  size_t code_reloc_size = 0;
  // do some read(s) */
  lseek(fd, 0, SEEK_SET);
  
  // read ELF header
  read(fd, (void *)&ehdr, sizeof(ElfW(Ehdr)));
  // read section name string table
  // first, read its header
  lseek(fd, ehdr.e_shoff + ehdr.e_shstrndx * sizeof(ElfW(Shdr)), SEEK_SET); // 
  read(fd, (void *)&shdr, sizeof(ElfW(Shdr)));
  // next, read the section, string data
  SectNames = __alloca(shdr.sh_size);
  lseek(fd, shdr.sh_offset, SEEK_SET);
  read(fd, SectNames, shdr.sh_size);
  lseek(fd, fsize - 10, SEEK_SET); //sizeof(".rela.text") == 10

  char label[11] = {0};
  read(fd, label, 10);
  assert (StrCmp(".rela.text", label) == 0);
  lseek(fd, fsize - 10 - sizeof(size_t), SEEK_SET);
  read(fd, &code_reloc_size, sizeof(size_t));
  lseek(fd, fsize - 10 - sizeof(size_t) 
     - code_reloc_size, SEEK_SET);
  size_t alloc_size = (code_reloc_size / GLRO(dl_pagesize) + 1) * GLRO(dl_pagesize);
  ElfW(Rela) *code_relocs = (ElfW(Rela) *)__mmap ((caddr_t) 0, 
      alloc_size,
      PROT_READ|PROT_WRITE, 
      MAP_ANONYMOUS|MAP_PRIVATE,
      -1, 0);
  //ElfW(Rela) *code_relocs = (ElfW(Rela) *)__alloca(code_reloc_size);
  if (code_relocs)
    read(fd, (char *)code_relocs, code_reloc_size);

  const char *codeSecNames = ".plt|.init|.text|.fini";
  //const char *codeSecNames = ".plt|.init|.text|.fini|__libc_freeres_fn\
  //   |__libc_thread_freeres_fn|.gcc_except_table|__libc_subfreeres       \
  //   |__libc_atexit|__libc_thread_subfreeres"; 
  //got.plt is also included, as it cannot 
  //be modified ans thus should be write-protected
  //const char *dataSecNames = ".got.plt"; 

  // read all section headers
  off_t codeStartOff = fsize, codeEndOff = 0;
  off_t gotpltOff = 0; 
  size_t gotpltSize = 0;
  off_t pltOff = 0; 
  size_t pltSize = 0;
  ElfW(Addr) gotpltAddr = 0;
  remap_info->l_addr = old_l_addr;
  //off_t relapltOff = 0; 
  //size_t relapltSize = 0;
  ...
  bool hasAllCode = false;
  // get all code sections and got.plt section
  for (idx = 0; idx < ehdr.e_shnum; idx++)
  {
    const char* sname = "";
    lseek(fd, ehdr.e_shoff + idx * sizeof(ElfW(Shdr)), SEEK_SET);
    read(fd, (void *)&shdr, sizeof(ElfW(Shdr)));
    // print section name
    // FIXME: this implementation is problematic
    if (shdr.sh_name) {
      sname = SectNames + shdr.sh_name;
      if (StrStr(codeSecNames, sname) && !hasAllCode) {
        if (shdr.sh_offset < codeStartOff) 
          codeStartOff = shdr.sh_offset;
        else if (shdr.sh_offset >= codeEndOff)
          codeEndOff = shdr.sh_offset + shdr.sh_size;
      ...
      // we also remap got.plt tables and have
      // its offset from code sections fixed
      else if (strcmp(".got.plt", sname) == 0) {
        gotpltOff = shdr.sh_addr; //note this is different from sh_offset
        remap_info->oldGpBase = old_l_addr + gotpltOff;
        remap_info->gotpltSize = shdr.sh_size;
        gotpltAddr = shdr.sh_addr;
      }
      //else if (strcmp(".rela.plt", sname) == 0) {
      //  relapltOff = shdr.sh_offset;
      //  relapltSize = shdr.sh_size;
      //}
      else if (strcmp(".rodata", sname) == 0) {
        rodataOff = shdr.sh_offset;
        rodataSize = shdr.sh_size;
      }
      else if (strcmp(".data.rel.ro", sname) == 0) {
        relRoOff = shdr.sh_addr;
        relRoSize = shdr.sh_size;
      }
      else if(codeEndOff > 0) //suppose code sections are continious
        hasAllCode = true;
    }
  }
  remap_info->oldCodeBase = old_l_addr + codeStartOff;
  // restore fd
  lseek(fd, ori_off, SEEK_SET);
  off_t codepad = codeStartOff % GLRO(dl_pagesize);
  remap_info->codeSize = codeEndOff - codeStartOff;
  remap_info->relRoSize = relRoSize;
  remap_info->oldRelRoBase = old_l_addr + relRoOff;
  size_t sizeWithRodata = remap_info->codeSize;
  if (rodataOff >= codeEndOff)
    sizeWithRodata += (rodataOff - codeEndOff + rodataSize);

  // map code sections to new random address
  remap_info->newCodeBase = (ElfW(Addr)) __mmap ((void *) 0, 
      sizeWithRodata + codepad, //.rodata is attached
      // writable is required as we need for code relocation, 
      // which will be removed later
      PROT_READ|PROT_WRITE, 
			MAP_COPY|MAP_FILE,
			fd, codeStartOff - codepad) + (ElfW(Addr)) codepad;

  // .rodata is mapped together with code sections
  remap_info->newRelRoBase = remap_info->newCodeBase - 
    remap_info->oldCodeBase + remap_info->oldRelRoBase;

	off_t gppad = gotpltOff % GLRO(dl_pagesize);
  // map .got.plt section to new random address
  remap_info->newGpBase = (ElfW(Addr)) __mmap ((void *) 0, remap_info->gotpltSize + gppad,
					      PROT_READ|PROT_WRITE,
                MAP_ANON|MAP_PRIVATE,
                -1, 0) + (ElfW(Addr)) gppad;
					      //fd, gotpltOff - gppad) + (ElfW(Addr)) gppad;

  // update offsets in data-access instructions
  ElfW(Rela) *reloc;
  const int byteNum = 4; //FIXME: suppose 4 bytes of updated offset
  for (size_t i = 0; i < code_reloc_size / sizeof(ElfW(Rela)); ++i) {
    // the offset in the relocation entry is virtual address
    // from the base of the file
    reloc = code_relocs + i;
    ElfW(Addr) reloc_addr = reloc->r_offset - codeStartOff + remap_info->newCodeBase;
    ElfW(Addr) ori_value = *((int *)reloc_addr) + reloc_addr + byteNum;
    // do nothing if the address is for code
    // FIXME: suppose it is based on rip value
    // ignore relocation for .rodata
    if (remap_info->newCodeBase <= ori_value && 
        ori_value < remap_info->newCodeBase + remap_info->codeSize) {
      continue;
    }
#ifdef DO_STATISTICS
    // number of data-access patches
    ++(*(stat_ptr+3));
#endif
    if ((remap_info->newCodeBase + sizeWithRodata - rodataSize) <= ori_value && 
        ori_value < (remap_info->newCodeBase + sizeWithRodata))
    {
      // FIXME: ignore jump table
      continue;
    }
    ElfW(Addr) offset_adjust = 0;
    //if (*pNewGpAddr <= ori_value && ori_value < *pNewGpAddr + gotpltSize) 
    //  offset_adjust = (*pNewCodeAddr - (old_l_addr + codeStartOff)) - 
    //    (*pNewGpAddr - (old_l_addr + gotpltAddr));
    //else
      offset_adjust = remap_info->newCodeBase - (old_l_addr + codeStartOff);
    
    // TODO: code/data separation is disable due to unreliability (see NOTE.todo) 
#ifndef ENABLE_DEBUG_MODE
    *((int *)reloc_addr) -= (int)offset_adjust;
#endif
  }


  // do relocation for plt section
  ElfW(Addr) plt_offset_adjust = 0;
  //if ((*(int *)safeMemReg) == 0)
    plt_offset_adjust = remap_info->newCodeBase - (old_l_addr + codeStartOff);
  //else
  //  plt_offset_adjust = *pNewCodeAddr - (old_l_addr + codeStartOff)
  //    - (*pNewGpAddr - (old_l_addr + gotpltAddr)); 
  //TODO:need to support randomized got.plt
  ElfW(Addr) newPltAddr = remap_info->newCodeBase + pltOff - codeStartOff;
  ElfW(Addr) reloc_plt = (ElfW(Addr))newPltAddr;
  const int plt_ent_size = 16;
  *((int *)(reloc_plt + 2)) -= (int) plt_offset_adjust; //first plt entry
  *((int *)(reloc_plt + 8)) -= (int) plt_offset_adjust; //first plt entry
  reloc_plt += plt_ent_size;
  int plt_ent_num = (pltSize - plt_ent_size) / plt_ent_size;
  for (int i = 0; i < plt_ent_num; ++i) {
    *((int *)(reloc_plt + i * plt_ent_size + 2)) 
      -= (int) plt_offset_adjust; 
	}

  // make code sections non-writable
  __mprotect((void *)(remap_info->newCodeBase - codepad), 
      remap_info->codeSize + codepad, PROT_READ|PROT_EXEC);

#ifdef ENABLE_DEBUG_MODE
  // ag-note: for debugging
  remap_info->newCodeBase = remap_info->oldCodeBase;
  remap_info->newGpBase = remap_info->oldGpBase;
  remap_info->newRelRoBase = remap_info->oldRelRoBase;
#endif
  munmap(code_relocs, alloc_size);
  /************************************************************************/

  return true;
}

// ag-note: remap the code and gotplt of the given module
off_t  _dl_remap_module (
    int fd, const char *fname, 
    ElfW(Addr) old_l_addr ) {

#ifdef USE_ASLR_GUARD
  ...
  char label[11] = {0};
  read(fd, label, 10);
  //only remap module with .rela.text section
  ...
  if( _dl_remap_code_gotplt(fd, old_l_addr, &remap_info)) {
    //extern void *safeMemReg; 
    *(ElfW(Addr) *)(safeMemReg + sizeof(int) + 
        (*(int *)safeMemReg) * sizeof(struct remapInfo)) = 
      old_l_addr;
    *(ElfW(Addr) *)(safeMemReg + sizeof(int) + 
      (*(int *)safeMemReg) * sizeof(struct remapInfo) + sizeof(ElfW(Addr))) = 
      remap_info.oldCodeBase;
    *(ElfW(Addr) *)(safeMemReg + sizeof(int) + 
      (*(int *)safeMemReg) * sizeof(struct remapInfo) + 2 * sizeof(ElfW(Addr))) = 
      remap_info.newCodeBase;
    *(ElfW(Addr) *)(safeMemReg + sizeof(int) + 
      (*(int *)safeMemReg) * sizeof(struct remapInfo) + 3 * sizeof(ElfW(Addr))) = 
      remap_info.oldGpBase;
    *(ElfW(Addr) *)(safeMemReg + sizeof(int) + 
      (*(int *)safeMemReg) * sizeof(struct remapInfo) + 4 * sizeof(ElfW(Addr))) = 
      remap_info.newGpBase;
    *(ElfW(Addr) *)(safeMemReg + sizeof(int) + 
      (*(int *)safeMemReg) * sizeof(struct remapInfo) + 5 * sizeof(ElfW(Addr))) = 
      remap_info.codeSize;
    *(ElfW(Addr) *)(safeMemReg + sizeof(int) + 
      (*(int *)safeMemReg) * sizeof(struct remapInfo) + 6 * sizeof(ElfW(Addr))) = 
      remap_info.gotpltSize;
    *(ElfW(Addr) *)(safeMemReg + sizeof(int) + 
      (*(int *)safeMemReg) * sizeof(struct remapInfo) + 7 * sizeof(ElfW(Addr))) = 
      remap_info.oldRelRoBase;
    *(ElfW(Addr) *)(safeMemReg + sizeof(int) + 
      (*(int *)safeMemReg) * sizeof(struct remapInfo) + 8 * sizeof(ElfW(Addr))) = 
      remap_info.newRelRoBase;
    *(ElfW(Addr) *)(safeMemReg + sizeof(int) + 
      (*(int *)safeMemReg) * sizeof(struct remapInfo) + 9 * sizeof(ElfW(Addr))) = 
      remap_info.relRoSize;

    //make original code non-executable
    //cannot make loader non-executable until control is transfered to 
    //remaped loader
#ifndef ENABLE_DEBUG_MODE
    if ((*(int *)safeMemReg) > 0) { 
      off_t codepad = remap_info.oldCodeBase % GLRO(dl_pagesize); 
      __mprotect((void *)(remap_info.oldCodeBase - codepad),  
          remap_info.codeSize + codepad, PROT_READ);
    }
#endif

    (*(int *)safeMemReg) += 1;
#ifdef PRINT_DEBUG_INFO
    _dl_debug_printf("@ l_addr: 0x%lx\
        new code addr: 0x%lx old code addr: 0x%lx  code moved: 0x%lx  code size: 0x%lx\n\
        new gotplt addr: 0x%lx  old gotpltaddr: 0x%lx  gotplt moved: 0x%lx\n\
        new rodata addr: 0x%lx  old rodata addr: 0x%lx  rodata size: 0x%lx\n", 
        old_l_addr, remap_info.newCodeBase, remap_info.oldCodeBase, 
        remap_info.newCodeBase - remap_info.oldCodeBase, 
        remap_info.codeSize, remap_info.newGpBase, remap_info.oldGpBase, 
        remap_info.newGpBase - remap_info.oldGpBase,
        remap_info.newRelRoBase, remap_info.oldRelRoBase, remap_info.relRoSize);
#endif
  }
  if (fname != NULL)
    fd = -1;
...
off_t _dl_remap_loader(
  const char *fname,
  ElfW(Addr) old_l_addr) {

  /* parse the target elf file again to get the loader path */
  int fd;
  const ElfW(Ehdr) ehdr;
  const ElfW(Shdr) shdr;
  int idx = 0;
  ...
  read(fd, (void *)&shdr, sizeof(ElfW(Shdr)));
  // next, read the section, string data
  snames = __alloca(shdr.sh_size);
  lseek(fd, shdr.sh_offset, SEEK_SET);
  read(fd, snames, shdr.sh_size);
  ...
  for (idx = 0; idx < ehdr.e_shnum; idx++) {
    ...
    read(fd, (void *)&shdr, sizeof(ElfW(Shdr)));
    if (shdr.sh_name) {
      sname = snames + shdr.sh_name;
      if (strcmp(sname, ".interp") == 0) {
        ld_path = __alloca(shdr.sh_size);
        lseek(fd, shdr.sh_offset, SEEK_SET);
        read(fd, ld_path, shdr.sh_size);

#ifdef PRINT_DEBUG_INFO
        _dl_debug_printf("remap loader: %s\n", ld_path);
#endif
        ret = _dl_remap_module(0, ld_path, old_l_addr);
        break;
      }
    }
  }
...
#ifdef PRINT_DEBUG_INFO
  _dl_debug_printf("\n@ name: %s l_map_start: 0x%lx mapstart: 0x%lx\n",
    name,  l->l_map_start, c->mapstart);
#endif
...
#ifdef USE_ASLR_GUARD
  // ag-note: remap libraries
  _dl_remap_module(fd, NULL, l->l_addr);
#endif

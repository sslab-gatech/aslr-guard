...
__libc_dlsym_private (struct link_map *map, const char *name)
{
  ...
  if (! dlerror_run (do_dlsym_private, &sargs))
    return adjust_encode_code_addr(DL_SYMBOL_ADDRESS (sargs.loadbase, sargs.ref), MAY_ENCODE, 0);
  return NULL;
}
...
__libc_dlsym (void *map, const char *name)
{
  ...
  args.name = name;
  void *result;
  // ag-note: encrypt the code pointer (address of the symbol)
#ifdef SHARED
  if (__builtin_expect (_dl_open_hook != NULL, 0))
    return adjust_encode_code_addr( _dl_open_hook->dlsym (map, name), MAY_ENCODE, 0);
#endif
  result = (dlerror_run (do_dlsym, &args) ? NULL
	  : (void *) (DL_SYMBOL_ADDRESS (args.loadbase, args.ref)));
  return adjust_encode_code_addr(result, MAY_ENCODE, 0);
}
